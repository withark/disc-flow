...
- 관리자 주소: <https://withark.github.io/disc-flow/?admin=1>
...
6. 공개 사이트의 관리자 화면에서 발급된 `/exec` 주소와 3단계 관리자 비밀번호를 입력합니다.
7. 로그인 후 `검사 링크 복사`를 눌러 참여자에게 공유합니다. 이 링크에는 공개 웹앱 주소만 포함되고 관리자 비밀번호는 포함되지 않습니다.
...
GitHub Pages에서는 Apps Script 웹앱이 결과 저장소 역할을 합니다. Apps Script 주소는 참여 링크에 포함되지만 관리자 비밀번호는 포함되지 않습니다. 기존 비공개 서버 배포에서는 웹 앱 주소와 토큰을 서버 환경에만 저장하며, `Sheets 동기화` 버튼으로 기존 D1 결과를 Record ID 기준으로 추가하거나 갱신할 수 있습니다.
